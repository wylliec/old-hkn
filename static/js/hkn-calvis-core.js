...
calvis.Calendar.prototype.updateWeekView = function() {
  ...
  this.overlayHKN(firstDate, lastDate);
};
...
calvis.Calendar.prototype.appendEventHKN = function(id, event) {
  ...
  if (cellHeight < currentContentHeight + contentFontHeight + bottomMargin) {
    ...
    moreDiv.bind('click', function() {
      calendar.currentPivotDate = event["start_time"]
      calendar.currentControlView = calendar.initWeekView;
      calendar.initWeekView();
    });
...
calvis.Calendar.prototype.createFittingSpan = function() {
  var span = jQuery('<span id="fitting_span"/>');
  span.addClass(calvis.contentCellClass);
  span.css({'visibility': 'hidden'});
  jQuery('#content').append(span);
 ...
 */ 
calvis.Calendar.prototype.overlayHKN = function(startDate, endDate) {
  var calendar = this;

  var feedUri = "/event/feed/";
  ...
  var callback = function(eventEntries) {    
  ...
  jQuery.get(feedUri, data, callback, "json");
};
...
calvis.Calendar.prototype.fitText = function(text, limit) {
  ...
  while (currentWidth > limit) {
    ...
    if(calvis.debug && !confirm("currentWidth: " + currentWidth + " text: " + text)){
        return text;
    }
  }
