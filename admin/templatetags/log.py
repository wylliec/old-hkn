...
from hkn.admin.models import LogEntry
    ...
    def render(self, context):
        if self.user is None:
            ...
            if not self.user.isdigit():
                self.user = context[self.user].pk
            context[self.varname] = LogEntry.objects.filter(user__pk__exact=self.user).select_related()[:self.limit]
        return ''
