...
NOTE_FONT_ID = "F5_0"
TABLE_HEADER_FONT_ID = "F4_0"
DEPARTMENT_HEADER_FONT_ID = "F5_0"

department_header_x = None
...
def is_department_header_cell(cell):
	global department_header_x
	return cell.parentNode.getAttribute("id") == DEPARTMENT_HEADER_FONT_ID \
			and are_close(atof(cell.getAttribute("x")), department_header_x, 2)		
	
def is_note_cell(cell):
	global department_header_x
	return cell.parentNode.getAttribute("id") == NOTE_FONT_ID \
			and not are_close(atof(cell.getAttribute("x")), department_header_x, 2)
...
def getDepartmentXml(department_name, cells, headers, doc):
...
def get_transformed_xml_document(dom):
	...
	department_header_x = atof(cells[0].getAttribute("x"))
	...
	for i, department_index in enumerate(department_indicies):
		...
		dept = getDepartmentXml(department_name, cells[department_index+1:next_department_index], headers, doc)
