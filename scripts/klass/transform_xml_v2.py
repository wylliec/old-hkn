...
def is_department_header_cell(cell):
	return cell.parentNode.getAttribute("id") == DEPARTMENT_HEADER_FONT_ID \
		and atof(cell.getAttribute("x")) < 50
	
def is_table_header_cell(cell):
	previous_font = cell.parentNode.previousSibling
	if previous_font == None:
		return False
	while previous_font.nodeName != "font":
		previous_font = previous_font.previousSibling
	return cell.parentNode.getAttribute("id") == TABLE_HEADER_FONT_ID \
		and previous_font.getAttribute("id") == DEPARTMENT_HEADER_FONT_ID
...
def is_v2(dom):
	...
	# version 2 for fall2004, spring 2005, summer 2005
	if yr == 2004 and season.lower() == "fall":
		return True
	if yr == 2005 and (season.lower() == "spring" or season.lower() == "summer"):
		return True
	return False 	
